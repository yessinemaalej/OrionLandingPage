...
import { Activity, Package, Tag, Wallet, Search, Clock, XCircle, Truck, User, DollarSign } from "lucide-react";
...
const StatCard = ({ title, value, icon: Icon, className = "" }: { title: string; value: string | number; icon: any; className?: string }) => (
  <div className={`bg-gray-800 rounded-xl p-6 shadow-lg border border-purple-500/20 ${className}`}>
...
const UserCard = ({ user, onUpdateStatus }: { user: User; onUpdateStatus: (address: string) => void }) => {
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-purple-500/20">
      <div className="flex justify-between items-start mb-4">
        ...
        <span className={`px-3 py-1 rounded-full text-sm ${getStatusColor(user.shipmentStatus)}`}>
          {user.shipmentStatus}
        </span>
      </div>
      <div className="space-y-3 text-gray-300">
        ...
        <div className="pt-4 border-t border-gray-700">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <p><span className="text-gray-400">Email:</span> {user.shipmentDetails.email}</p>
            <p><span className="text-gray-400">Phone:</span> {user.shipmentDetails.phoneNumber}</p>
            <p><span className="text-gray-400">Country:</span> {user.shipmentDetails.country}</p>
            <p><span className="text-gray-400">City:</span> {user.shipmentDetails.city}</p>
            <p className="col-span-2">
              <span className="text-gray-400">Address:</span> {user.shipmentDetails.addressLine1}
              {user.shipmentDetails.addressLine2 && `, ${user.shipmentDetails.addressLine2}`}
            </p>
          </div>
        </div>
        <button
...
const TransferOwnershipCard = ({ onTransfer }: { onTransfer: (address: string) => void }) => {
    const [newOwner, setNewOwner] = useState("");
  
    return (
      <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-purple-500/20">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-300">Transfer Ownership</h3>
  ...
  const UpdatePriceCard = ({ onUpdate }: { onUpdate: (price: string) => void }) => {
...
const AdminContent = () => {
  ...
  const contractAddress = "0x72D0737feDDbdFe3D144fe3E0cFE1BBff4b1D189";
    ...
    try {
      ...
      const formattedPromoCodes = codes
        .map((code: string, index: number) => ({
          code,
          discount: typeof discounts[index] === 'bigint' ? Number(discounts[index]) : discounts[index],
        }))
  ...
  const updatePrice = async (newPrice: string) => {
    try {
      ...
      const contract = await connectToContract();
      const transaction = await contract?.updatePaymentAmount(ethers.parseEther(newPrice));
      await transaction.wait();
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => 
      user.shipmentDetails.email.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const stats = useMemo(() => {
    ...
    const inProgress = users.filter(u => u.shipmentStatus === 'In Progress').length;
    const notShipped = users.filter(u => u.shipmentStatus === 'Not Shipped').length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-gray-900 to-black text-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
              <StatCard title="Total Orders" value={users.length} icon={Package} />
              <StatCard title="Active Promo Codes" value={promoCodes.length} icon={Tag} />
              <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-purple-500/20">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-gray-300">Contract Balance</h3>
                  <Wallet className="text-purple-400" size={24} />
                </div>
                <p className="text-4xl font-bold mb-4">{contractBalance} DIONE</p>
                <button
                ...
                {withdrawStatus && (
                  <p className={`mt-2 ${withdrawStatus.includes("successfully") ? "text-green-400" : "text-red-400"}`}>
                    {withdrawStatus}
                  </p>
                )}
              </div>
        ...
        <div>
          ...
          {transferStatus && (
            <p className={`mt-2 text-center ${transferStatus.includes("successfully") ? "text-green-400" : "text-red-400"}`}>
              {transferStatus}
            </p>
          )}
        </div>
        <div>
          ...
          {updatePriceStatus && (
            <p className={`mt-2 text-center ${updatePriceStatus.includes("successfully") ? "text-green-400" : "text-red-400"}`}>
              {updatePriceStatus}
            </p>
          )}
        </div>
          ...
          <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-purple-500/20">
            ...
            <div className="grid gap-4">
              {promoCodes.map((promo, index) => (
                ...
                >
                  <div>
                    <span className="text-lg font-semibold text-purple-400">{promo.code}</span>
                    <span className="ml-4 text-gray-400">{promo.discount}% discount</span>
                  </div>
                  <button
